...
from .forms import NoteForm, SponsorForm, GamesForm, NoteFormUpdate
from .models import Note, Sponsor, Games
...
class NoteByGames(ListView):
    model = Note
    
    queryset = Note.objects.all()
    def get_queryset(self):
        Gamess = self.kwargs['Games']
        pieces = Gamess.split('/') #extract different Gamess separated by /
        
        queries = [Q(Games__title__iexact=value) for value in pieces]
        ...
        allnotes = Note.objects.filter(user=curruser).filter(query).distinct().order_by('Games__title')
        self.queryset = allnotes #Setting the queryset to allow get_context_data to apply count
        return allnotes
    
    def get_context_data(self, **kwargs):
        context = super(NoteByGames, self).get_context_data(**kwargs)
        context['total'] = self.queryset.count()
        context['curruser'] = UserProfile.objects.get(user=self.request.user)
        return context
...
class MyView(TemplateView):
    Sponsor_form_class = SponsorForm
    Games_form_class = GamesForm
    ...
    def get(self, request, *args, **kwargs):
        kwargs.setdefault("createSponsor_form", self.Sponsor_form_class())
        kwargs.setdefault("createGames_form", self.Games_form_class())
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "btn_createSponsor" in request.POST['form']:
            form = self.Sponsor_form_class(**form_args)
            if not form.is_valid():
                return self.get(request,
                                   createSponsor_form=form)
            else:
                form.save()
        ...
        elif "btn_createGames" in request.POST['form']:
            form = self.Games_form_class(**form_args)
            if not form.is_valid():
                return self.get(request,
                                   createGames_form=form)
            else:
                form.save() #save the new object
                data = Games.objects.all() # retrieve all records
                result_list = list(data.values('id','title'))
